        ...
        estreeWalker.walk ast,
            enter: (node, parent) ->
                ...
                if node.type is 'Identifier'
                    if node.name of uses and not uses[node.name] and isReference(node, parent) and not scope.contains(node.name) and node.name isnt 'global' # diff: global doesn't mean it's commonJS
                        uses[node.name] = true
                    return
