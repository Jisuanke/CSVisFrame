...
if (global.PointerEvent) {
    ...
    for (var name of list) {
        $.event.fixHooks[
            (events[name] = 'MSPointer' + _.capitalize(name))
        ] = hook;
    }
}
...
export default function Panzoom(elem, options) {
    ...
    var $empty = $();
    for (var name of ['$zoomIn', '$zoomOut', '$zoomRange', '$reset']) {
...
$.extend(Panzoom.prototype, {
    ...
    zoom(scale, opts) {
        ...
        // Set the scale
        matrix[0] = scale;
        matrix[3] = _.isNumber(options.dValue) ? options.dValue : scale;

        // Calling zoom may still pan the element
        this.setMatrix(matrix, {
            animate: _.isBoolean(options.animate) ? options.animate : animate,
    ...
    _bind() {
        ...
        for (var key of ['Start', 'Change', 'Zoom', 'Pan', 'End', 'Reset']) {
        ...
        if ($zoomIn.length && $zoomOut.length) {
            ...
            $zoomIn.on(str_click, (e) => {
                e.preventDefault();
                this.zoom();
            });
            $zoomOut.on(str_click, (e) => {
                e.preventDefault();
                this.zoom(true);
            });
        }
        ...
        if ($zoomRange.length) {
            ...
            events[(pointerEvents ? 'pointerdown' : 'mousedown') + ns] = () => {
                this.transition(true);
            };
            // Zoom on input events if available and change events
            // See https://github.com/timmywil/jquery.panzoom/issues/90
            events[(supportsInputEvent ? 'input' : 'change') + ns] = () => {
    ...
    _trigger(event, ...args) {
        ...
        this.$elem.triggerHandler(event, [this, ...args]);
    },
    ...
    _startMove(event, touches) {
        ...
        if (touches && touches.length === 2) {
            ...
            move = (e) => {
                ...
                var middle = this._getMiddle(touches = e.touches);
                var diff = this._getDistance(touches) - startDistance;

                // Set zoom
                this.zoom(diff * (options.increment / 100) + startScale, {
                    focal: middle,
                    matrix: matrix,
                    animate: false
                });
    ...
    if (_.isString(options)) {
        ...
        this.each(function() {
            ...
            if (!instance) {
                ...
                _.isFunction(m = instance[options]) &&
